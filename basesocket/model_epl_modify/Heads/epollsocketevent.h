...
public:
    ...
    void eventLoop(){
	    ...
	    while (1) {
    	    // 检查是否有事件发生
			log_msg("start to epoll_wait");
	        res =  epoll_wait(_iepfd, events, MAX_SOCKET_EVENTS , -1);
			log_msg("%d events ", res);
    	    eventHandle(events, res);
			log_msg("after finish handle event");

        }
		log_msg("jump out of while(1) loop");
}
		...
		for (int i=0;i < cnt; i++)
		{
			...
			if ((tfd == _listenfd) &&(events[i].events & EPOLLIN)){
				...
				if (_count >= 1) {
					...
					vecTasks.clear();
					_count--;
					log_msg("rm event; close fd; --count");
					continue;
				}
				else {
					_count++;
				...
				else if (vecRmtHeads.empty() && vecTasks.empty()) {
					cmpVecInfos(vecLocHeads, vecRmtHeads, vecTasks);
					log_msg("vecLocHeads size: %ld\nvecRmtHeads size: %ld\nvec\
								DiffHeads size: %ld\n", vecLocHeads.size(), 
								vecRmtHeads.size(),	vecTasks.size());
...
private:
	...
	int _count = 0;
	std::vector<struct Head> vecTasks; // 对比远端和本地生成的任务列表
	DataBuffer & pdbuf;
};
